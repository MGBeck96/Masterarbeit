...
typedef struct tagEMotor {
  ...
  uint8_t isEnabled; //Motor control is enabled
  uint32_t speed_motor; //Motor control velocity(Number of pulse of per second)
} EMotor;
...
extern int SetEndEffectorSuctionCup(bool ctrlEnabled, bool suck, bool isQueued, uint64_t *queuedCmdIndex);
...
*********************************************************************************************************/
extern int ProtocolClearAlarmState();
extern int GoHome();
extern int GetPose();
extern int SetWait(uint32_t timeout);
extern int SetEMotor(EMotor *motorCMD, uint64_t *queuedCmdIndex);
#endif
